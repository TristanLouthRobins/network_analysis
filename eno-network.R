...
nodes <- read_csv("data/e-nodes.csv")
edges <- read_csv("data/e-edges.csv")
...
ggraph(g, layout = "with_kk") + # kk = force-directed layout (the Kamada-Kawai layout)
  geom_edge_link() +
  geom_node_point(aes(size = degree, color = n_listeners)) +
  geom_node_text(aes(label = name, size = 5), nudge_y = .10, repel = TRUE) +
  theme_graph()

ggraph(g, layout = "with_kk") + # kk = force-directed layout (the Kamada-Kawai layout)
  geom_edge_link() +
  geom_node_point(aes(size = between, color = n_listeners)) +
  geom_node_text(aes(label = name, size = 5), nudge_y = .10, repel = TRUE) +
  theme_graph()
...
# Adjacency matrix
A <- as_adjacency_matrix(g, attr = NULL, names = F, sparse = F)
...
as_adjacency_matrix(g, attr = NULL)
